...
const fetchMetadata = async (displayName) => {
    try {
        ...
        if (response.data) {
            ...
            console.error('No metadata found for this site.');
            return null;
        }
    } catch (error) {
...
// Shared function to initialize dates
    ...
    const fetchTableData = async (tableName, serverName, page = 1) => {
        ...
        try {
            // Step 1: Fetch the date range first
            const dateRangeResponse = await fetch(`/api/summary_table/date_range?serverName=${serverName}&tableName=${tableName}`);
            const dateRangeData = await dateRangeResponse.json();

            // Check if the date range fetch was successful
            if (dateRangeResponse.ok) {
                ...
                // Use fetched date range data to format start and end dates
                const defaultStartDate = new Date(dateRangeData.start_date);
                const defaultEndDate = new Date(dateRangeData.end_date);
        ...
        try {
            ...
            if (response.ok) {
                ...
                const existingCombinations = new Set(
                    data.map(item => `${item.aggregated_timestamp.split('T')[0]}|${item.display_field_name}`)
                );
                ...
                allDates.forEach(date => {
                    variables.forEach(variable => {
                        ...
                        if (existingCombinations.has(key)) {
                            const existingEntry = data.find(item => item.aggregated_timestamp.split('T')[0] === date && item.display_field_name === variable);
                            filledData.push(existingEntry);
                        } else {
                            filledData.push({
                                aggregated_timestamp: `${date}T00:00:00.000Z`,
...
// Automatically select all servers when the modal opens
    ...
    const fetchAggregatedDataAvailability = async (interval = 'Daily') => {
        ...
        const serverParam = selectedServers.join(','); // Include only selected servers
        ...
        try {
            ...
            if (response.ok) {
                ...
                const existingCombinations = new Set(
                    data.map(item => `${item.aggregated_timestamp.split('T')[0]}|${item.display_server_name}|${item.display_table_name}`)
                );
                    ...
                    variables.forEach(variable => {
                        ...
                        if (existingCombinations.has(key)) {
                            const existingEntry = data.find(item =>
                                item.aggregated_timestamp.split('T')[0] === date &&
                                item.display_server_name === serverName &&
                                item.display_table_name === tableName
                            );
                            filledData.push({
...
// Updated download button function
    ...
    const renderTableData = (data) => {
        ...
        const fieldNames = Array.from(
            new Set(
                data.flatMap((row) =>
                    row.field_values.map((fv) => fv.display_field_name.trim())
                )
            )
        ).sort();
        ...
        return (
            <div className="summarydata-modal-content">
                ...
                <div className="summarydata-pagination-controls">
                    ...
                    <button className="summarydata-button" onClick={nextPage}>
                        →
                    </button>
                    <span>
                    Page Size:{' '}
                    ...
                    <span>
                    Total Pages: {Math.ceil(totalRows / pageSize) || 1}
                </span>
                </div>
                <div className="summarydata-table-container">
                    <table className="summarydata-data-table">
                        <thead>
                        <tr>
                            <th className="summarydata-timestamp-column">
                                Timestamp
                            </th>
                        ...
                        <tr>
                            <th></th>
                            {fieldNames.map((fieldName) => (
                                <th key={`${fieldName}-unit`}>
                                    {fieldUnits[fieldName]}
                                </th>
                            ))}
                            <th></th>
                            <th></th>
                        ...
                        {data.map((row, index) => (
                            <tr key={index}>
                                <td className="summarydata-timestamp-column">
                                    {row.timestamp}
                                </td>
                                {fieldNames.map((fieldName) => {
                                    const fieldValue = row.field_values.find(
                                        (fv) =>
                                            fv.display_field_name.trim() ===
                                            fieldName
                                    );
                                    return (
                                        <td key={fieldName}>
                ...
                >
                    <div className="summarydata-consent-modal-content">
                        ...
                        <p>
                            By downloading this data, you agree to cite the dataset
                            using the DOI provided in the downloaded file. Proper
                            citation ensures the data is traceable and credits are
                            given appropriately for the resources used. The DOI
                            refers to all tables and variables for this site.
                        </p>
                        <div className="summarydata-consent-checkbox">
                            ...
                            />
                            <label>
                                I agree to cite this dataset according to the
                                provided DOI.
                            </label>
                        </div>
...
// Function to determine node color based on the end_date
    ...
    const fetchAndSetMetadata = async (displayName, tableName) => {
        ...
        if (metadata && metadata.doi) {
            setMetadataLinks(prevLinks => ({
                ...prevLinks,
                [tableName]: metadata.doi
            }));
        }
    };

    useEffect(() => {
        // Fetch metadata links for all tables when the component loads
        servers.forEach(server => {
            tables[server.display_server_name]?.forEach(table => {
                fetchAndSetMetadata(server.display_server_name, table.display_table_name);
            });
        });
    }, [servers, tables]);
...
// Function to open the consent modal
    const openConsentModal = (tableName, serverName) => {
    ...
    const handleConsent = () => {
        if (consentGiven) {
            ...
            if (pendingDownloadTable && pendingDownloadServer) {
                ...
                downloadData(pendingDownloadTable, pendingDownloadServer); // Proceed with the download
            } else {
        ...
        {dateRanges[`${server.display_server_name}-${table.display_table_name}`] ? (
            dateRanges[`${server.display_server_name}-${table.display_table_name}`].error ? (
                ...
                `${new Date(dateRanges[`${server.display_server_name}-${table.display_table_name}`].start_date).toLocaleDateString()} - ${new Date(dateRanges[`${server.display_server_name}-${table.display_table_name}`].end_date).toLocaleDateString()}`
                                    ...
                                    {/* Metadata Link */}
                                    <div className="tooltip">
                                        {metadataLinks[table.display_table_name] ? (
                                            <a
                                                href="#"
                                                onClick={(e) => {
                                                    e.preventDefault(); // Prevent the default anchor behavior
                                                    handleViewMetadataClick(metadataLinks[table.display_table_name]);
                                                }}
            ...
            >
                <div className="modal-header">
                    <div className="macos-window-controls">
                        <div className="macos-button close" onClick={() => closeCustomModal()}></div>
