...
function VasiScienceCentreAWSHourly() {
    ...
    const metaData = {
        name: [
            "Time", "BattV_Min", "BP_kPa", "WS_ms_S_WVT", "WindDir_D1_WVT",
            "WindDir_SD1_WVT", "AirTC_Avg", "RH", "SlrW_Avg", "CUV5_W_Avg",
            "Rain_mm_Tot", "T107_C_Min", "T107_C_Avg", "VW_Avg"
        ],
        units: [
            "", "Volts", "kPa", "meters/second", "Deg", "Deg", "Deg C", "%",
            "W/m^2", "W/m^2", "mm", "Deg C", "Deg C", ""
        ]
    };

    useEffect(() => {
        const fetchURL = showRecent
            ? '/api/get_vasi_science_centre_aws_hourly_data?limit=365'
            : '/api/get_vasi_science_centre_aws_hourly_data';
        fetch(fetchURL)
    ...
    const fetchRowCount = () => {
        fetch('/api/get_vasi_science_centre_aws_gethourlycount')
            .then(response => response.json())
    ...
    const handleDownloadCSV = () => {
        window.location.href = '/api/download_vasi_science_centre_aws_hourly_csv';
    };
...
export default VasiScienceCentreAWSHourly;
